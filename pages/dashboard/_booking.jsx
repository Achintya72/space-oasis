...
import content from "../checkout/content.json";
...
export default function Booking({ booking, i }) {
  ...
  const handleChange = (d) => {
    changeUser((prev) => {
      prev.bookings[i].arrival = new Date(
        d.valueOf() -
          prev.bookings[i].arrival.valueOf() +
          prev.bookings[i].arrival.valueOf()
      );
  ...
  const removeBooking = () => {
    changeUser((prev) => {
      prev.miles += (booking.cost * 0.75 * 1000000) / 100;
  ...
  return (
    <div
      ...
      key={(booking?.departure ?? new Date()).toISOString()}
    >
      <div className={styles.picAndTime}>
        <Image
          src={`/static/packages/${booking?.img}`}
          alt={booking?.type}
          fill
        />
      ...
      <div className={styles.editable} style={{ borderRadius: "0px" }}>
        ...
        >
          <div className={styles.labelInput}>
            ...
            <Dropdown
              options={options}
              selected={options.indexOf(booking?.meal ?? "Veg.")}
              changeSelected={handleMealChange}
            />
          </div>
          <div className={styles.buttons}>
            <DateModal
              startDate={booking?.departure ?? new Date()}
              endDate={
                new Date(
                  (booking?.departure ?? new Date()).getFullYear() + 2,
                  (booking?.departure ?? new Date()).getMonth(),
                  (booking?.departure ?? new Date()).getDate()
                )
              }
